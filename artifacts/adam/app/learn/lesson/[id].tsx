...
export default function LessonPlayer() {
  ...
  // Build step list:
  // hook → introduce(each word) → repeat(each word) → see(all) → challenge(3) → celebrate
  ...
  const onAnswer = (chosenIdx: number, correctIdx: number) => {
    if (chosenIdx === correctIdx) {
      setFeedback("ok");
      setStars((s) => s + 1);
      speak(lang === "ar" ? "أحسنت" : "Great job", voice).catch(() => {});
      setTimeout(() => {
        setFeedback("");
        setStepIdx((i) => i + 1);
      }, 900);
  ...
  const finish = () => {
    saveProgress((p) => {
      ...
      const lessonsCompleted = alreadyDone
        ? p.lessonsCompleted
        : [...p.lessonsCompleted, lesson.id];
      return {
        ...p,
        lessonsCompleted,
        ...
        englishLessons:
          isArabicLesson || alreadyDone ? p.englishLessons : p.englishLessons + 1,
        arabicLessons:
          !isArabicLesson || alreadyDone ? p.arabicLessons : p.arabicLessons + 1,
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.background }} edges={["top"]}>
      ...
      >
        ...
        <Text style={{ fontWeight: "800", color: c.text, fontSize: 14 }}>
          ⭐ {stars}
        </Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, gap: 14, flexGrow: 1 }}>
        ...
        >
          {lessonTitle(lesson, lang).toUpperCase()} · {t("step")} {stepIdx + 1} {t("of")}{" "}
          {steps.length}
        </Text>
        ...
        >
          {step.type === "hook" && (
            <SoftCard color={c.yellow}>
              <Text style={{ fontWeight: "800", fontSize: 13, color: "#5B3700" }}>
                ✨ {t("funFact")}
              </Text>
              <Text
                style={{
              ...
              <View style={{ marginTop: 12 }}>
                <SpeakButton
                  text={lang === "ar" ? lesson.funFactAr : lesson.funFactEn}
                  voice={voice}
                />
              </View>
            </SoftCard>
          ...
          {step.type === "introduce" && step.data && (
            <SoftCard>
              ...
              >
                <Text style={{ fontSize: 100 }}>{step.data.emoji}</Text>
                <Text
                  style={{ fontSize: 36, fontWeight: "800", color: c.primary }}
                >
                  {isArabicLesson ? step.data.ar : step.data.en}
                </Text>
                <Text style={{ fontSize: 16, color: c.mutedForeground }}>
                  {isArabicLesson ? step.data.en : step.data.ar}
                </Text>
                <SpeakButton
                  text={isArabicLesson ? step.data.ar : step.data.en}
                  voice={voice}
                  size={56}
                />
              </View>
              ...
              >
                {lesson.words.map((w) => (
                  <Pressable
                    key={w.en}
                    onPress={() =>
                      speak(isArabicLesson ? w.ar : w.en, voice).catch(() => {})
                    }
                    style={({ pressed }) => ({
                      width: 90,
            ...
            <SoftCard>
              ...
              >
                ...
                <Text style={{ color: c.primary }}>
                  {isArabicLesson
                    ? step.data.correct.ar
                    : step.data.correct.en}
                </Text>
                {lang === "ar" ? "؟" : "?"}
              </Text>
              <View
              ...
              >
                {step.data.options.map((opt: any, idx: number) => (
                  <Pressable
                    key={opt.en + idx}
                    onPress={() =>
                      onAnswer(idx, step.data.options.indexOf(step.data.correct))
                    }
                    style={({ pressed }) => ({
                      width: 110,
                      height: 110,
